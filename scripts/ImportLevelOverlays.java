...
public class ImportLevelOverlays extends GhidraScript {
	...
	public void run() throws Exception {
		...
		boolean confirmation = askYesNo(
				"WARNING",
				"This script is quite destructive (it deletes existing memory blocks), and is only intended for use on new files. Do you want to continue?");
		if (!confirmation) {
			return;
		}
		...
		// Delete the frontend sections as these will be replaced with an
		// overlay block.
		destroyBlock(".lit");
		destroyBlock(".bss");
		destroyBlock(".data");
		destroyBlock("lvl.vtbl");
		destroyBlock("lvl.camvtbl");
		destroyBlock("lvl.sndvtbl");
		destroyBlock(".text");
		
		// Import the frontend.
		importOverlay("frontend", assetBank, "boot_elf.elf");
		
		// Import the frontbin.
		File frontBinDir = new File(assetBank.getPath() + "/globals/frontbin");
		importOverlay("frontbin", frontBinDir, "frontbin.elf");
		...
		for (File level : levels.listFiles()) {
			if (level.isDirectory()) {
				boolean success = importOverlay("lvl" + level.getName(), level, "overlay.elf");
				if (!success) {
					...
					for (File actualLevel : level.listFiles()) {
						if (actualLevel.isDirectory()) {
							importOverlay("lvl" + actualLevel.getName(), actualLevel, "overlay.elf");
						}
					}
				}
			}
	...
	public void destroyBlock(String name) throws Exception {
	...
	public boolean importOverlay(String outputName, File directory, String fileName) throws Exception {
		FileSystemService fileSystemService = FileSystemService.getInstance();
		
		File overlayFile = new File(directory.getPath() + "/" + fileName);
		...
		try {
			...
			parseOverlay(outputName, provider);
		} finally {
			if(provider != null) {
				provider.close();
			}
	...
	public void parseOverlay(String outputName, ByteProvider provider) throws Exception {
		...
		// works with sections instead of segments. If we're loading the
		// frontend from the boot ELF, we need some additional logic to remove
		// the core sections.
		long blockAddress;
		long blockOffset;
		long blockSize;
		ElfProgramHeader segment = elfHeaders.getProgramHeaders()[0];
		if (outputName.equals("frontend")) {
			ElfSectionHeader section = elfHeaders.getSection(".lit");
			if (section == null) {
				throw new Exception("Cannot find .lit section.");
			}
			blockAddress = section.getAddress();
			blockOffset = section.getOffset();
			blockSize = (segment.getOffset() + segment.getMemorySize()) - section.getOffset();
		} else {
			blockAddress = segment.getVirtualAddress();
			blockOffset = segment.getOffset();
			blockSize = segment.getMemorySize();
		}
		...
		for (ElfSectionHeader section : elfHeaders.getSections()) {
			if (section.getAddress() >= blockAddress) {
				String label = outputName + section.getNameAsString();
	...
	public void createCameraFunctions(String outputName, Address start, Address end) throws Exception {
	...
	public void createSoundFunctions(String outputName, Address start, Address end) throws Exception {
